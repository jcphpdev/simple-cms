      ...
      <div class="row justify-content-center">
        <div class="col-md-5">
          <div class="card-group">
            <div class="card p-4">
              <div class="card-body">
                <h1>{{ __('messages.PasswordConfirmation') }}</h1>
                @if (session('status'))
                <div class="alert alert-success">
                    {{ session('status') }}
                </div>
                @else
                <p class="text-muted">{{ __('messages.PasswordConfirmationMessage') }}</p>
                @endif
                <form method="POST" action="{{ route('password.confirm') }}">
                  ...
                  <div class="row">
                    <div class="col-6">
                      <button class="btn btn-primary px-4" type="submit">{{ __('messages.Confirm') }}</button>
                    </div>
                  </div>
                </form>
